<script lang="ts">
  ...
  function handleConstantHeadingInput(event: Event) {
  ...
  function handleConstantHeadingBlur(event: Event) {
    const target = event.currentTarget as HTMLInputElement;
...
<select
  ...
  class="rounded-md bg-neutral-100 dark:bg-neutral-950 dark:border-neutral-700 border-[0.5px] focus:outline-none w-24 sm:w-28 text-sm"
...
{:else if endPoint.heading === "constant"}
  <div class="flex items-center gap-1 flex-wrap">
    ...
    <input
      ...
      on:input={handleConstantHeadingInput}
      on:blur={handleConstantHeadingBlur}
